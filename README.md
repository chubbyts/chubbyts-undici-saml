 ...
 * **Rejected saml messages:** The actual reason (wrong signature, expired, wrong audience, ...) is only logged (level `info`) via the optional logger, never sent to the client (`403`). Errors not related to the saml message (unreachable identity provider, ...) are rethrown, so your error handling responds with a `5xx`.
