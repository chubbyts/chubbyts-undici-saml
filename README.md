 ...
 * **Custom parts:** Each part is replaceable: `IdpMetadataResolver` is `() => Promise<IdpMetadata>`, `SamlServiceProvider` is `{ resolveLoginUrl, verifySamlResponse }`, `SamlSession` is `{ resolveIdentity, createCookie, createRemovalCookie }`, `SamlAssertionIdStore` is `{ consume }` (see replay). Throw an `InvalidSamlResponseError` (`@chubbyts/chubbyts-undici-saml/dist/error`) within a custom `verifySamlResponse` to get the `403` response, any other error is rethrown.
